...
const sounds = {
    0: new Audio("grafika/sounds/cube_turn_0.mp3"),
    1: new Audio("grafika/sounds/cube_turn_1.mp3"),
    2: new Audio("grafika/sounds/cube_turn_2.mp3"),
    3: new Audio("grafika/sounds/cube_turn_3.mp3"),
    4: new Audio("grafika/sounds/cube_turn_4.mp3"),
    5: new Audio("grafika/sounds/cube_turn_5.mp3"),
    6: new Audio("grafika/sounds/cube_turn_6.mp3"),
    7: new Audio("grafika/sounds/cube_turn_7.mp3"),
    8: new Audio("grafika/sounds/cube_turn_8.mp3"),
    9: new Audio("grafika/sounds/cube_turn_9.mp3"),
    10: new Audio("grafika/sounds/cube_turn_10.mp3"),
    11: new Audio("grafika/sounds/cube_turn_11.mp3"),
    12: new Audio("grafika/sounds/cube_turn_12.mp3"),
    13: new Audio("grafika/sounds/cube_turn_13.mp3"),
    14: new Audio("grafika/sounds/cube_turn_14.mp3"),
    15: new Audio("grafika/sounds/cube_turn_15.mp3"),
    16: new Audio("grafika/sounds/cube_turn_16.mp3"),
    done: new Audio("grafika/sounds/neverhood_done.mp3")
}
...
function animate() {
    ...
    if (isMoving === false && currentMove === null && movesQueue.length > 0) {
        if (delay > 0) {
            ...
            // }
            sounds[useIdx].play()
            if (movesDone !== undefined) {
                movesDone += 1
                turnsRemaining.innerText = movesDone
            }
            if (movesQueue.length === 0 && movesDone !== undefined) {
                // right after cube animation is solved
                setTimeout(() => {
                    sounds.done.play()
                }, 500)
                setTimeout(() => {
                    bigButton.classList.remove("hide")
                    scrambleBtn.classList.remove("hide")
                    scrambleBtn.innerText = "SCRAMBLE AGAIN"
                    solveBtn.classList.add("hide")
                    info.classList.add("hide")
                    scrambling.classList.add("hide")
                    solving.classList.add("hide")
                }, 2500)
            } else if (movesQueue.length === 0 && movesDone === undefined) {
                // right after scrambling is done
                // TO DO: replace movesDone with a better if condition
                setTimeout(() => {
                    bigButton.classList.remove("hide")
                    solveBtn.classList.remove("hide")
                    scrambleBtn.classList.add("hide")
                    info.classList.add("hide")
                    scrambling.classList.add("hide")
                    solving.classList.add("hide")
                }, 500)
    ...
    if (isMoving === true && currentMove !== null) {
        // rotator.rotation[currentMove.dir] += .3 * currentMove.lor // adding too small number breaks the geometry
        rotator.rotation[currentMove.dir] += tempo[0] * currentMove.lor // adding too small number breaks the geometry
    ...
    // origin.rotation.x -= 0.005
    // origin.rotation.y -= 0.005
...
function test() {
    if (seq === 0) {
        ...
        scrambling.classList.remove("hide")
        bigButton.classList.add("hide")
        
        movesDone = undefined
        const arr = getSolvingMoves()
        for (let i = 0; i < arr.length; i++) {
            movesQueue.push(arr[i])
        }
        seq++
    } else if (seq === 1) {
        info.classList.remove("hide")
        scrambling.classList.add("hide")
        solving.classList.remove("hide")
        bigButton.classList.add("hide")

        solve()
        movesDone = 0
        const arr = getSolvingMoves("skipScrambling")
        const turnsLen = arr.length
        turnsAll.innerText = turnsLen
        turnsRemaining.innerText = 0
        for (let i = 0; i < arr.length; i++) {
            movesQueue.push(arr[i])
        }
        seq = 2
...
//         test()
//     } else {
//         if (origin.rotation.x === 0) {
//             origin.rotation.x -= Math.PI/3
//         } else {
//             origin.rotation.x = 0
//         }
//     }   
...
function handleButtonClick(e) {
    ...
    adjustSpeed(e.target.id)
}

function handleBigButton(e) {
    test()
...
document.querySelector(".bigButton").addEventListener("click", handleBigButton)
